...
Resource Allocation for Collector
...
# 3000 = 0.03
echo 100000 | sudo tee /sys/fs/cgroup/cpu/nodrop_collector/cpu.cfs_period_us
echo 3000  | sudo tee /sys/fs/cgroup/cpu/nodrop_collector/cpu.cfs_quota_us
```

```
# Copy scripts
cp ./scripts/ctrl/fetch_stream.sh ./build/scripts/ctrl/fetch_stream.sh
cp ./scripts/ctrl/analyze_stream.sh ./build/scripts/ctrl/analyze_stream.sh
...
# Collecting
sudo cgexec -g cpu,cpuset:nodrop_collector ./scripts/ctrl/fetch_stream.sh 0.03 /tmp/nodrop

# After collecting
./scripts/ctrl/ctrl stat > /tmp/stat_after.txt
./scripts/ctrl/analyze_stream.sh /tmp/nodrop/stream.idx /tmp/nodrop/stream.raw /tmp/stat_before.txt /tmp/stat_after.txt
```
